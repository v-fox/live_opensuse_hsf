...
var cysSmallSaveDialog = {
	params: null, persist_: null, persistInterval_: null, currentFile_: null, videoID : null, ttsurl: null, cc_asr: null, subtitleTry:0, cssFilesList : [], cssUrlsList : [],	cssIndex : 0, doComments : true, avatarCount : 0,
   progressLabel : null, progressMax : 450,  workTimer : 0, maxDL : 1, comments : null, stats : null, subtitle: null, page : null, sspacer : '<!-- cys_stats_script -->', commentList : [], gBrowserParent:null,
...
//cysCommons.cysDump("\n\n*********Here I got it::"+cysSmallSaveDialog.params.detail);
   ...
   addStats : function (stats, cb) {
      if (cb) {
         if (stats) { //dump('\n*** Stats data:\n'+stats+'\n\n'); cysSmallSaveDialog.writeStringToFile(cysSmallSaveDialog.getFile('stats.xml'),null,stats,'UTF-8');  // saves stats info to file
            if (stats.length<30 || /<error_message>.+<\/error_message>/.test(stats)) {
               ...
               sdata = cysSmallSaveDialog.statsScript.replace('ytChData =***','ytChData = '+sdata); cysSmallSaveDialog.statsScript = sdata;
            }  //dump('\n\n*** Stats info:\ndata:\n' + sdata + '\n\nhtml:\n'+ shtml + '\n\n');
         ...
         var mdir = cysSmallSaveDialog.getFile('MData'), statUrl = 'http://www.youtube.com/insight_ajax?action_get_statistics_and_data=1&v='+cysSmallSaveDialog.params.videoID;
         ...
         if (typeof cysSmallSaveDialog.params.js1 == 'string' && typeof cysSmallSaveDialog.params.js2 == 'string') {
            var f1 = mdir.clone(), f2 = mdir.clone(); f1.append('js'); f2.append('cc')
            cysCommons.writeFile(f1,cysSmallSaveDialog.params.js1,'UTF-8'); cysCommons.writeFile(f2,cysSmallSaveDialog.params.js2,'UTF-8'); //dump('\n* Writing libraries...\n');
         }
         cysCommons.getAJAX(statUrl,cysSmallSaveDialog.addStats,'POST',cysSmallSaveDialog.stoken);
		  } else {
         cysSmallSaveDialog.stats = 'done'; cysSmallSaveDialog.downloadingMainPage();
      }
   },
    ...
    addComments : function (comments, cb, id) {
        ...
        cysSmallSaveDialog.comments = cysCommons.getComments(cysSmallSaveDialog.params.saveCommentsCount,cysSmallSaveDialog.params.gBrowserParent);
   ...
   addAvatars : function (d, cb, i) { if (!cb) {return;} //dump('\n Avatars to get: '+cysSmallSaveDialog.avatarCount);
      var dd; try{dd = JSON.parse(d); dd = dd.entry.gphoto$thumbnail.$t}catch(e){}
	...
	downloadingMainPage : function (){
      if (cysSmallSaveDialog.persist_.currentState!=3) return;
      ...
      if (this !== cysSmallSaveDialog) {setTimeout(function(){cysSmallSaveDialog.downloadingMainPage()},20); /*dump('\n\n*** this !== cysSmallSaveDialog !!! (downloadingMainPage)\n\n');*/ return;}
		try{
         ...
         if (cysSmallSaveDialog.params.saveComments) {   // save comments
             if(cysSmallSaveDialog.comments){
                 if (cysSmallSaveDialog.comments!='done') {
                    ...
                    if (ctgt) {
                        cysSmallSaveDialog.page = cysSmallSaveDialog.page.replace(ctgt[0],cysSmallSaveDialog.comments); /*dump('\n\n*** Comment block found and replaced:\n'+ctgt[0]+'\n\ncomment data:\n'+cysSmallSaveDialog.comments);*/
                    }
         ...
         if (cysSmallSaveDialog.params.saveStat) {      // save stats
            if (cysSmallSaveDialog.stats) {
               ...
               cysSmallSaveDialog.addStats(); return;
            }
         }
         
         cysSmallSaveDialog.setProgress(95, cysCommons.getCysString("saving.data"));
         var pageStr = cysSmallSaveDialog.processPage (cysSmallSaveDialog.page,"MData"), tt, ytlink = 'http://www.youtube.com/watch?v='+cysSmallSaveDialog.params.videoID;
        ...
        /*if (cysSmallSaveDialog.params.saveStat && typeof cysSmallSaveDialog.params.stathtml != 'undefined' && cysSmallSaveDialog.params.stathtml !='') {  
         ...
         cysSmallSaveDialog.page = pageStr; cysCommons.writeStringToFile(cysSmallSaveDialog.currentFile_, null, pageStr);
	...
	processPage : function (page, folder, mainpage){
		var head = page.indexOf("<head>"), t, tt; if (head!=-1) page = page.substring(0,head+6)+'<meta http-equiv="content-type" content="text/html; charset=UTF-8">'+page.substring(head+6);
		try{
  			var stime = new Date().getTime(), n = 0;
         ...
         if (mainpage) {
            ...
            if (folder) page= cysSmallSaveDialog.findLink(page, /\.js|\.jpg|\.ico|\.gif|\.png|\.swf/gi, folder, 2000);
      ...
      var begin, start, beginTargetChar, endTargetChar, result;
		do {
         ...
         if (linkFileName==lastlinkFileName){    
			...
			originalChartLink = "";
         var linkFile = cysCommons.getFile([cysSmallSaveDialog.currentFile_.parent.path, folder]);
			...
			if( !linkFile.exists()) {
				if (str=='.css'){
               	...
               	cysSmallSaveDialog.cssFilesList.push(linkFileName);
						cysSmallSaveDialog.cssUrlsList.push(unescape(link));
					}
