  ...
  doCancel : function (){
	...
	doOK : function (){
            ...
            try{
                    ...
                    if( !dir.exists() || !dir.isDirectory() ) dir.create(Ci.nsIFile.DIRECTORY_TYPE, parseInt("0777", 8));
                    ...
                    if (projDirIndex)	pname.value = pname.value + "-"+projDirIndex;
            }catch(ex){
      ...
      //          }
           // }
            ...
            if (cysSaveDialog.listmode=='history' || document.getElementById('folderlist').selectedIndex==-1 || cysCommons.cysPrefs.getBoolPref('folders.custom.track')) {
               ...
               if (fl.length >= cysSaveDialog.maxfolders) fl.pop()
               var a = path + '**' + fl.join('**')
            ...
            if (document.getElementById("save-dta").hasAttribute('checked')) cysSaveDialog.params.useDTA = true;
	...
	saveDialog_onload : function (){
            ...
            if (cysCommons.cysPrefs.getIntPref('folders.custom') == 1 ) cysSaveDialog.listmode = 'custom';
            cysSaveDialog.setFolderList(cysSaveDialog.listmode);
            if (cysCommons.DTAstatus()) {                           // DTA checkbox
               document.getElementById("dta-row").hidden = false; document.getElementById("dta-row-sep").hidden = false;
      ...
      if (cysSaveDialog.params.videoUrl) {
         ...
         if (cysSaveDialog.params.size) t += ('    -    ' + cysSaveDialog.params.size);
         document.getElementById("format").value = t;
         if (cysSaveDialog.params.convert!=null) {
            if (cysSaveDialog.params.convert) document.getElementById("cys-format-label").value = 'Audio:';
         }
//       } else {   EYVD sniffer
...
//          cysSniffer.regListener();
      ...
      if (!panel.hidden) toggle.setAttribute('class', 'cys-toggle-up');
...
//       }
   ...
   setFolderList : function (mode, openlist) {
      ...
      fl.removeAllItems()
      if (folders) {
         var f = folders.split('**', cysSaveDialog.maxfolders)
         for each (var k in f) {
            if (!k) break;
            fl.appendItem();
            fl.getItemAtIndex(fl.itemCount-1).setAttribute('label', k);
            fl.getItemAtIndex(fl.itemCount-1).setAttribute('tooltiptext', k);
            fl.getItemAtIndex(fl.itemCount-1).setAttribute('crop', 'center');
         }
      }
      if (flv) fl.value = flv;
      if (!fl.value && dldir) fl.value = dldir;
      if (!fl.value && defdir) fl.value = defdir;
      if (!fl.value) fl.value = cysCommons.getDefaultDir(cysSaveDialog.defaultSaveDir,false);
      fl.setAttribute('tooltiptext', fl.value);
      fl.setAttribute('crop', 'center');
      if (mode=='history') {
      ...
      if (openlist) fl.menuBoxObject.openMenu(true);
   },
   ...
   getCommentCount : function (thisdoc, docText) {   // try to find the number of comments in page
      ...
      if (d.className == 'comments-disabled') {
         ...
         for each (h in d.getElementsByTagName('h4')) {
            if (typeof h.innerHTML==='undefined') continue;
	...
	getCommentPages : function (doc) { dump('\n\nParsing comment count......\n\n');
