...
try { Cu.import("resource://gre/modules/PrivateBrowsingUtils.jsm"); } catch(e) {}
try { 
...
var downloadedFile = "",downloadedExt="", advancedFFmpedcommandCalled = 0, openedFolder=0;
...
var cysCommons = {
    ...
    convertFormatMatch: {1:[141,172,45,22,46,37,38],2:[141,172,45,22,43,44],3:[141,172,43,44,34,18],
                        11:[141,172,45,22,46,37,38],12:[141,172,45,22,43,44],13:[141,172,43,44,34,18],14:[141,172,171,140,43,44,34,18],
                        21:[141,172,45,22,46,37,38,43,44,34,18],
                        31:[22,37,38,85,84],33:[34,35],34:[18],
                        41:[141],42:[140],43:[139],
                        51:[22,37,38,85,84],53:[34,35],54:[18],
                        61:[141],62:[140],63:[139],
                        71:[172,45,46,102],72:[171,43,44,100],81:[251],82:[250],83:[249]},
    convertFormatMatch1:{1:[141,172,45,46,22,37,38],2:[141,172,45,22,43,44],3:[141,172,43,44,34,18],
                        11:[141,172,45,46,22,37,38],12:[141,172,45,22,43,44],13:[141,172,43,44,34,18],14:[141,172,171,140,43,44,34,18],
                        21:[141,172,45,46,22,37,38,43,44,34,18],
                        32:[22,37,38,85,84],33:[34,35],34:[18],
                        41:[141],42:[140],43:[139],
                        52:[22,37,38,85,84],53:[34,35],54:[18],
                        61:[141],62:[140],63:[139],
                        71:[172,45,46,102],72:[171,43,44,100],81:[251],82:[250],83:[249]},
    convertFormatMatch2:{1:[141,172,45,46],2:[141,172,45,43,44,22,37],3:[141,172,43,44,22,18,34],
                        11:[141,172,45,46],12:[141,172,45,43,44,22,37],13:[141,172,43,44,22,18,34],14:[141,172,171,140,43,44,22,18,34],
                        21:[141,172,45,46,43,44,22,18,34],
                        33:[22,37,38,85,84],34:[18,34],
                        41:[141],42:[140],43:[139],
                        53:[22,37,38,85,84],54:[18,34],
                        61:[141],62:[140],63:[139],
                        71:[172,45,46,102],72:[171,43,44,100],81:[251],82:[250],83:[249]},
    ...
    getFormats : function (quick, defaults) {
        ...
        if (quick) {
            ...
            if (cysCommons.canConvert) dformats = dformats.concat(['sep'],cysCommons.convertOutputs);
            fstr = cysCommons.cysPrefs.getCharPref('menu.quickmenu.order');
        } else {
            for (i=0;i<cysCommons.validFormats.length;i++) dformats.push('Fmt'+cysCommons.validFormats[i]);  // default list - complete!
            if (cysCommons.canConvert) {
               dformats.push('sep');
               for (i=0;i<cysCommons.convertFormats.length;i++){ var fmt = cysCommons.convertFormats[i]; if(fmt==11 || fmt==21 || fmt==31 || fmt==41 || fmt == 51 || fmt==61 || fmt==71 || fmt == 81){dformats.push('sep');}dformats.push('Cnv'+cysCommons.convertFormats[i]); }
            }
            fstr = cysCommons.cysPrefs.getCharPref('menu.dropdown.order');
        }
        if (fstr && !defaults){ dformats = cysCommons.mergeArrays(dformats, fstr.split(',')); } // merge options not in custom but in default into returned array 
        return dformats;
    },

    getSubMenu : function () {
       var def = '1,2,3,4,5'; if (cysCommons.canConvert) def += ',sep,6,7,8,9';
       var menus = def.split(','), mstr = cysCommons.cysPrefs.getCharPref('menu.dropdown.submenu.order');
       if (mstr) menus = cysCommons.mergeArrays(menus, mstr.split(','));
       return menus;
    },

    mergeArrays: function (def, custom) {  // merge additional options from defaults array into custom array
          var pf = custom.slice(0), df, i;
          for (i=0;i<pf.length;i++){if (pf[i].substr(0,1)=='-') pf[i] = pf[i].substr(1);}  // consider formats disabled by user...
          for each (df in def){if (pf.indexOf(df)==-1 && df!='sep') custom.push(df);}
          return custom;
    },
    ...
    executeInSandbox:function(scrp,gBrowser1){
        ...
        try{
            ...
            cysCommons.cysDump("error in sandbox:"+scrp)
        }
        return false;
    },
    ...
    getComments: function(cc,gBrowser1){
        ...
        var scrp="var commentele = document.getElementById('watch-discussion'); if(commentele) { var m = document.getElementsByClassName('comment-entry'); if(m) { mlen = m.length; if(mlen>"+cc+") mlen="+cc+"; if(mlen>0) { var html = \"\";for(var i=0;i<mlen;i++) { html = html+'<div class=\"comment-entry\">'+m[i].innerHTML+'</div>'; } } else ''; } else ''; }  else { ''; }";
        var allreview ='var d = document.getElementsByClassName("all-comments"); if(d){ if(d[0]) { d[0].innerHTML;} else{ ""; } } else "";';
        var commentshtml = '<div id="watch-discussion" class="branded-page-box yt-card scrolldetect"><div class="all-comments">'+(cysCommons.executeInSandbox(allreview,gBrowser1))+'</div><div class="comments embedded"><div id="" data-url="" class="comments-list">'+(cysCommons.executeInSandbox(scrp))+'</div></div></div><div id="cys-comments-sukhvir" style="display:none"></div>';
        return commentshtml;
    },
    ...
    getmaxfolders : function () {
       if (cysCommons.maxfoldercount) return cysCommons.maxfoldercount;
    ...
    cTime : function () {
       var t = new Date(); return t.getTime();
    },
    ...
    cysDump : function (x) { // combined console error reporting + logging
        var msg, debug = cysCommons.cysPrefs.getBoolPref('debug');
        if(debug){
            console.log(x);
        }
        if (typeof x == 'string') {
            if (debug) {msg = '\n[CYS] - '+x+'\n'; dump(msg); Cc['@mozilla.org/consoleservice;1'].getService(Ci.nsIConsoleService).logStringMessage(msg);}
        } else {
    ...
    getConverterPath : function () {
       ...
       var ff = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsILocalFile), ffname;
       var extdir = Cc[ "@mozilla.org/file/directory_service;1" ].getService(Ci.nsIProperties).get("ProfD", Ci.nsIFile), extdir2 = extdir.clone();
    ...
    getDefaultDir : function (defdir,isDefault){
            ...
            if (!dir){
                    try{
                            var ds = Cc[ "@mozilla.org/file/directory_service;1" ].getService(Ci.nsIProperties), t = '';
                            if (ds.has('DfltDwnld')) t = ds.get('DfltDwnld', Ci.nsIFile);
        if (!t && ds.has('Home')) t = ds.get('Home', Ci.nsIFile);
        if (!t && ds.has('Desk')) t = ds.get('Desk', Ci.nsIFile);
        if (!t) try{t = Cc["@mozilla.org/preferences-service;1"].getService(Ci.nsIPrefService).getBranch("browser.download.").getCharPref('lastDir');}catch(e){}
        if (!t && ds.has('TmpD')) t = ds.get('TmpD', Ci.nsIFile);
        if (t) {
           dir = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsILocalFile);
           dir.initWithPath((typeof t=='string')?t:t.path);
                            if( !dir.exists() || !dir.isDirectory()) dir.create(Ci.nsIFile.DIRECTORY_TYPE, parseInt("777", 8));
                            dir.append(defdir);
                            if( !dir.exists() || !dir.isDirectory()) dir.create(Ci.nsIFile.DIRECTORY_TYPE, parseInt("777", 8));
        }
                    }catch(ex){
    ...
    getPrivacy : function () {
       var win = cysCommons.getDoc().defaultView, privacy = {}; privacy.context = null, privacy.isprivate = false;
       if (win && typeof PrivateBrowsingUtils != "undefined" && PrivateBrowsingUtils.privacyContextFromWindow) {
          privacy.context = PrivateBrowsingUtils.privacyContextFromWindow(win);
    ...
    openTargetFolder:function(openFolderPath){
        try{
            ...
            cysCommons.cysDump("openTargetFolder:"+ex);
        }
    ...
    saveImagesToLocal : function (page, instr, folder,path, count){   // instr can be string or Regex object!
        ...
        var step = 0, rand, str, t;
        var lastlinkFileName;
        var chart_api_idx = 1;
        var begin, start, beginTargetChar, endTargetChar, result;
        do {
            step++;
            ...
            if (lastEndLinkPos==0) lastEndLinkPos = start;	
            ...
            if (endLinkPos==-1) break;
            link = page.substring(begin+1, endLinkPos);
            if (link==null || link.indexOf("http")==-1 || link.length>200) continue;
            var filenamestart, linkFileName;
            ...
            if (linkFileName.lastIndexOf(str)!=(linkFileName.length - str.length)) continue;
            ...
            if( !linkFile.exists()) {
                cysCommons.download(unescape(link),linkFile,true);
            }
        } while (step<count || endLinkPos>0);
        return page;
    },
    download: function (url1,file1,isHiddenDownload,useDTA,ref,convertTo,noCookie,url2,file2,ext) {//dump('\n\n* Downloading!!\nurl1: '+url1+'\nfile1: '+file1.path+'\nurl2: '+url2+'\nfile2: '+((typeof file2=='object')?file2.path:file2)+'\n');
      if (cysCommons.OS=='WINNT' && file1.path.length > 259) {    // Long path Windows alert!
         var PS = Cc["@mozilla.org/embedcomp/prompt-service;1"].getService(Ci.nsIPromptService); PS.alert(null, cysCommons.getCysString('name'), cysCommons.getCysString('pathtoolong'));
         return false;
      }
      if (useDTA) {if (cysCommons.doDTA(url1,file1,ref)===true) return null;}     //  try DTA if set, else download in browser
      // dump('\nReferrer for download:\n'+ref+'\n');
      const wp = Ci.nsIWebBrowserPersist; ref = null;
      var io_service = Cc["@mozilla.org/network/io-service;1"].getService(Ci.nsIIOService), privacy = cysCommons.getPrivacy(), ha = cysCommons.cysPrefs.getBoolPref('download.dash.hideaudio');
      if(openedFolder==0){
        cysCommons.openTargetFolder(file1.parent);
        openedFolder=1;
      }
      //cysCommons.cysDump("url2:"+url2+":::file2:"+file2+":::url1:"+url1+":::file1")
      if (url2 && file2) {convertTo = [file1.path,file2.path]; setTimeout(function(){ return dl(url1,file1,isHiddenDownload,ref,convertTo,noCookie,io_service,privacy,wp,ext);}, 1000); return dl(url2,file2,ha,ref,convertTo,noCookie,io_service,privacy,wp,ext) ;}
      ...
      return dl(url1,file1,isHiddenDownload,ref,convertTo,noCookie,io_service,privacy,wp,ext);

      function dl (url,file,isHiddenDownload,ref,convertTo,noCookie,io_service,privacy,wp,ext) { // download in browser
          ...
          if(typeof ext != 'undefined' && ext != null && Downloads.getList && xulAppInfo.name !='SeaMonkey'){//
          return Task.spawn(function () {
            ...
            try {
                ...
                //cysCommons.cysDump("error Task.spawn");
            }finally {
            }
          }).then(null, Components.utils.reportError);
      }else{
         try {
            var wbPersist = Cc["@mozilla.org/embedding/browser/nsWebBrowserPersist;1"].createInstance(wp), pl;
            wbPersist.persistFlags = wp.PERSIST_FLAGS_REPLACE_EXISTING_FILES | wp.PERSIST_FLAGS_BYPASS_CACHE
               ...
               pl = {
                  onLocationChange: function(a,b,c,d){transfer.onLocationChange(a,b,c,d)}, onSecurityChange: function(a,b,c){transfer.onSecurityChange(a,b,c)},
                  onStatusChange: function(a,b,c,d){transfer.onStatusChange(a,b,c,d)}, onProgressChange: function(a,b,c,d,e,f){transfer.onProgressChange(a,b,c,d,e,f)},
                  onStateChange:function(wp, req, fl, st){
                     transfer.onStateChange(wp, req, fl, st);
                     const nw = Ci.nsIWebProgressListener, nr = Ci.nsIRequest;
                     if (req && (fl & nw.STATE_START || fl & nw.STATE_STOP)) {
                        ...
                        if ((fl & nw.STATE_START ) && noCookie){           //try{var ck; ck = chn.getRequestHeader('Cookie');} catch(e){}; dump ('\nCookie was: '+ck+'\n');
                           chn.setRequestHeader('Cookie', '', false); req.loadFlags |= nr.LOAD_ANONYMOUS;
                        }
                        if (((fl & nw.STATE_STOP) && st==0 && chn.responseStatus<400) && !convertTo){cysCommons.applyAdvancedFFmpedCommand(file,ext);}
                        if ((fl & nw.STATE_STOP) && convertTo && st==0 && chn.responseStatus<400) cysCommons.doConversion(file, convertTo,ext);}
                  }
               }
            } else {
               pl =  {onLocationChange: function(a,b,c,d){}, onSecurityChange: function(a,b,c){}, onStatusChange: function(a,b,c,d){}, onProgressChange: function(a,b,c,d,e,f){},
                  onStateChange:function(wp, req, fl, st){
                     const nw = Ci.nsIWebProgressListener, nr = Ci.nsIRequest;
                     if (req && (fl & nw.STATE_START || fl & nw.STATE_STOP)) {
                        const chn = req.QueryInterface(Ci.nsIHttpChannel);
    ...
    applyAdvancedFFmpedCommand : function (file,ext){
        ...
        if(commandtemp=="") return;
        if (!cysCommons.convertPath) cysCommons.getConverterPath();
        if (!cysCommons.convertPath) return;
        ...
        setTimeout(function(){ffmpeg.runwAsync(commandarr,commandarr.length,cysCommons); cysOverlay.updateButtonTooltip();}, 500);
        
        
    },
    doConversion : function (file, fmt, ext) { // dump ('\n\n* doConversion\nformat: '+fmt+'\nfile: '+file.path);
        if (file==null || fmt==null) return;
        if (!cysCommons.convertPath) cysCommons.getConverterPath();
        if (!cysCommons.convertPath) return;
        var opt, files, fs, jobs;
        if (fmt instanceof Array) {
           if (typeof cysCommons.doConversion.jobs == 'undefined') cysCommons.doConversion.jobs = {};
           jobs = cysCommons.doConversion.jobs; fs = fmt.join();
           if (fs in jobs) {
              files = jobs[fs]; if (files[0][1] && files[1][1]) {/*dump('\n\ndoConversion stopped! This job set looks already complete...');*/ return;}
              if (file.path==fmt[0]) {files[0][1]=file;} else if (file.path==fmt[1]) {files[1][1]=file;}
              if (files[0][1] && files[1][1]) {fmt = 999;} else {/*dump('\n\ndoConversion stopped! Job set still incomplete?...');*/ return;}
           } else {
              jobs[fs] = [[fmt[0],0],[fmt[1],0]]; files = jobs[fs];
              if (file.path==fmt[0]) {files[0][1]=file;} else if (file.path==fmt[1]) {files[1][1]=file;}
              return;
           }
        }
        switch (parseInt(fmt)) {
           case 1:
              opt=['-vn','-acodec','libmp3lame','-aq','1']; break;
           case 2:
              opt=['-vn','-acodec','libmp3lame','-aq','3']; break;
           case 3:
              opt=['-vn','-acodec','libmp3lame','-aq','5']; break;
           case 11:
              opt=['-vn','-acodec','libmp3lame','-ab','256k']; break;
           case 12:
              opt=['-vn','-acodec','libmp3lame','-ab','192k']; break;
           case 13:
              opt=['-vn','-acodec','libmp3lame','-ab','128k']; break;
           case 14:
              opt=['-vn','-acodec','libmp3lame','-ac','1','-ar','32000','-ab','64k']; break;
           case 21:
              opt=['-vn']; break;
           /*case 247:
           case 242:
           ...
           case 999:
              ...
              if(files[0][1] && files[0][1].path.indexOf(".opus")>-1){
                opusformat = true;
              }
              if(files[1][1] && files[1][1].path.indexOf(".opus")>-1){
                opusformat = true;
              }
              opt=['-vcodec','copy','-acodec','copy']; break;
           default:
              opt=['-vn','-acodec','copy'];
        }
        try{                                             //dump ('\nStarting conversion. Format: '+fmt+'\noptions: '+opt);
           if (files) file = files[0][1];
           downloadedExt = ext;
           downloadedFile = file;
           ...
           setTimeout(function(){if (filet.exists()) {filet.remove(false); dump('\n\n'+filet.path+' still exists, deleting it now...\n\n');}},200);   // FF29 bug work-around!!
           cysCommons.conversions.push(file);

           var args=['-i',file.path]; if (files) {args.push('-i',files[1][1].path); cysCommons.conversions.push(files[1][1]); delete jobs[fs];}
           ...
           //args.push('-loglevel');
           //args.push('verbose');
           //args.push("-vstats_file");
           //args.push("testwesdfasd.txt");
           ...
           setTimeout(function(){ffmpeg.runwAsync(args,args.length,cysCommons); cysOverlay.updateButtonTooltip();}, 500);
        } catch(e){cysCommons.cysDump(e);}
    },

    observe : function(subject, topic, data){
        //cysCommons.cysDump(subject+"::topic:"+topic+"::::data:"+data);
       if (topic=='process-finished') {
          for each (var file in cysCommons.conversions) {
             try {
                file.remove(false);
                if (!file.exists()) delete cysCommons.conversions[cysCommons.conversions.indexOf(file)];
             } catch(e){}
          }
    ...
    doDTA : function (furl, file, ref) {   // download with DownThemAll!
        if (!cysCommons.DTAstatus()) return false;
        ...
        var dir = file.parent.path, ps = '//', dtadirs, dtanew, dtamask;
        try {
        ...
        if (ref != null) ref = new DTA.URL(IO.newURI(ref,'UTF-8',null));
        var dl = {fileName: file.leafName, title: ftext, url: furl, referrer: ref,  description: ftext};
        try {
        ...
        if (dtadirs != null && dtadirs != '') dtapref.setCharPref('directory', dtadirs);
        if (dtamask != null) dtapref.setCharPref('renaming', dtamask);      
        return true;
    },

    DTAstatus : function () {    // check if DTA is available 
       if (!cysCommons.cysPrefs.getBoolPref('dta.enabled')) return false;
       try {
           Cc["@mozilla.org/network/io-service;1"].getService(Ci.nsIIOService).newChannel("resource://dta/support/filtermanager.jsm", null, null);
           return true;
         } 
         catch(e) {}
    ...
    getAJAX : function (url,callBack,rtype,info,info2,info3) {//cysCommons.cysDump('\ngetAJAX - '+url);
       var req = new XMLHttpRequest(), rtype = rtype || 'GET', data;
                 req.open(rtype, url, true);
       req.onreadystatechange = function() {
         if (req.readyState==4){
             if (req.status==200) {
                if (rtype=='HEAD') {
                   if (info) {callBack(req.getResponseHeader(info), true, info2, info3);}else{callBack(req.getAllResponseHeaders(), true);}
                } else {
    ...
    getVideoFormatByUrl : function (url, longformat, convertTo, age){
        try{
         var fstr, convert, fmt, tt, t, d='';
         if (convertTo!=null) {                           // local conversion - ffmpeg
            convert = parseInt(convertTo);
            if (convert > 0) {url = ''; fmt = 'Cnv' + convert; fstr = cysCommons.getCysString(fmt+'.1');}
         }
         if (url) {
            if (!fmt) {t = /[?&]itag=(133|134|135|136|137|138|160|266|264|299|298)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(133|134|135|136|137|138|160|266|264|299|298)\D/.exec(url);
                }
               if (t) { fmt = 'Fmt' + t[1]; fstr = (cysCommons.canConvert)?'MP4':"MP4V"; if (convert===0) fstr = (cysCommons.cysPrefs.getBoolPref('download.aac.as.m4a'))?'M4A':'AAC';}}
            if (!fmt) {t = /[?&]itag=(139|140|141)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(139|140|141)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = "M4A"; }}
            if (!fmt) {t = /[?&]itag=(18|22|37|38|82|83|84|85|59|78)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(18|22|37|38|82|83|84|85|59|78)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = "MP4"; if (convert===0) fstr = (cysCommons.cysPrefs.getBoolPref('download.aac.as.m4a'))?'M4A':'AAC';}}
            if (!fmt) {t = /[?&]itag=(242|243|244|245|246|247|248|271|272|278|302)\D/.exec(url); 
                if(!t){
                  t = /[?\/]itag\/(242|243|244|245|246|247|248|271|272|278|302)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = (cysCommons.canConvert)?'WebM':"WebMV"; if (convert===0) fstr = "OGG";}}
            if (!fmt) {t = /[?&]itag=(43|44|45|46|100|101|102|303|308|313|315)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(43|44|45|46|100|101|102|303|308|313|315)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = "WebM"; if (convert===0) fstr = "OGG";}}
            if (!fmt) {t = /[?&]itag=(171|172)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(171|172)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = "OGG"; }}
            if (!fmt) {t = /[?&]itag=(34|35)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(34|35)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = "FLV"; if (convert===0) fstr = (cysCommons.cysPrefs.getBoolPref('download.aac.as.m4a'))?'M4A':'AAC';}}
            if (!fmt) {t = /[?&]itag=(5|6)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(5|6)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = "FLV"; if (convert===0) fstr = "MP3";}}
            if (!fmt) {t = /[?&]itag=(251|250|249)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(251|250|249)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = "OPUS"; }}
            if (!fmt) {t = /[?&]itag=(13|17|36)\D/.exec(url);
                if(!t){
                  t = /[?\/]itag\/(13|17|36)\D/.exec(url);
                }
               if (t) {fmt = 'Fmt' + t[1]; fstr = "3GP"; if (convert===0) fstr = (cysCommons.cysPrefs.getBoolPref('download.aac.as.m4a'))?'M4A':'AAC';}}
         ...
         return fstr;
          ...
          cysCommons.cysDump("error in getVideoFormatByUrl:"+ex);
       }
    },

    fmtFromUrl : function (a) {a = /[?&]itag=(\d+)\D/.exec(a); if(!a){a=/[?\/]itag\/(\d+)\D/.exec(a);} if (a) { return a[1];}else{return null;}},

    isYouTubeUrl: function(url,dir){
        if (url && dir) {dir = "^https?:\\/\\/(www\\.)?youtube\\.com/"+dir; if (url.match(dir)) return true;}
        return cysCommons.isYouTubeChannelUrl(url);
    },

    isYouTubeChannelUrl : function(url){
        var doc, t; if (!url) {doc = cysCommons.getDoc(); url = doc.location.href;} if (!url) return;
    ...
    getTitle : function(str){
        return cysCommons.fixProjectName(str || cysCommons.getDoc().title);
    },
    ...
    fixProjectName : function (str,fromSaveDialog){ //dump('\nstr: '+str);
        var len, R = /\.(MP4|FLV|3GP|WEBM|AVI|WMV|MOV|MPG|MPEG)$/i;
        ...
        if(newfilename !="" && fromSaveDialog != true){
            newfilename = newfilename.replace("[filename]",str);
            var d = new Date();
            ...
            str = newfilename.trim();
        }
        
        str = str.replace("- YouTube","").trim().replace("YouTube -","").trim();
        while (str.length != len) {str.replace(R, ''); len = str.length;}
        str = str.replace(/[\u200c-\u200f|\u2028-\u202e|\u25b6]/gi,'').replace(/["|\\|/|:|\*|\?|<|>|\|]/gi,replace_character_with).trim();//.replace(/\./g,replace_character_with)
        if (str.search(/&\w{2,}/)) str = cysCommons.entities2char(str);
	var su = str.toUpperCase();
	if (su=="CON" || su=="AUX" || su=="COM1" || su=="COM2" || su=="COM3" || su=="COM4" || su=="LPT1" || su=="LPT2" || su=="LPT3" || su=="PRN" || su=="NUL") str += replace_character_with;
        if (!str) str = "Title";
    ...
    getFile : function(fn) { // fn: string or array of strings
       try{
          var i, f = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsILocalFile);
          if (typeof fn=='string' && fn) {
             f.initWithPath(fn);
          } else if (fn instanceof Array) {
             f.initWithPath(fn[0]);
             if (fn.length>1) {for (i=1;i<fn.length;i++){f.append(fn[i]);};}
          }
          return f;
       }catch(ex){cysCommons.cysDump(ex);}
    },
    readFileToString : function (_file){
        if (!_file.exists()) {cysCommons.cysDump('\n!! readFileToString - file not found: '+_file.path+'\n'); return '';}
        var lines = [], charset = "UTF-8";
    ...
    writeStringToFile : function (_file, _newFile, _content, charset){
        if (_file.exists()) _file.remove(true);
    ...
    getFileNames : function (fp, fn, ext) { // fp: path (string); fn: filename (string); ext: array of file extensions;
       var file = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsILocalFile), file2, files = {}, x = 0, exists, i;
       for (i=0;i<ext.length;i++) {
          do {
                         file.initWithPath(fp); file.append(fn+(x?"-"+x:'')+"."+ext[i]); exists = file.exists(); if (exists) x++;
                 } while(exists)
       }
       files.fn = fn+(x?"-"+x:'');
       for (i=0;i<ext.length;i++) {file2 = file.clone(); file2.initWithPath(fp); file2.append(fn+(x?"-"+x:'')+"."+ext[i]); files['f'+i] = file2;}
       return files;
    },
    ...
    writeFile : function (file, str, charset){  // async file write - string to file
        ...
        var converter = Cc["@mozilla.org/intl/scriptableunicodeconverter"].createInstance(Ci.nsIScriptableUnicodeConverter); converter.charset = charset;
        var istream = converter.convertToInputStream(str), ostream = FileUtils.openSafeFileOutputStream(file);
    ...
    entities2char : function (str) {  // converts HTML entities to respective Unicode characters
       var d = cysCommons.getDoc().createElement('div');
       d.innerHTML = str; str = d.textContent; d = null;
       return str;
    },

    cloneObject : function (obj) {  // use: t = new cysCommons.cloneObject(object);
       for (var i in obj) {if (typeof obj[i] == 'obj') {this[i] = new cloneObject(obj[i]);} else {this[i] = obj[i];}}
    },
   ...
   getTimes : function (node){
      ...
      return {start: cysCommons.toTime(start), end: cysCommons.toTime(end)}
   },
    ...
    getVideoIdFromEmbeddedURL: function(match,url){
        try{
            ...
            if(match){
                ...
                if(temparr[0])  videoID = temparr[0];
                if(!videoID && url){
                    var temp;
                    if(url.indexOf("?v=")>-1){
                        temp = url.split("?v=");
            ...
            cysCommons.cysDump("error in getting videoID:"+ex);
        }
    }
}
