...
for interface in $(ls --color=never /sys/class/net/); do
	...
	case "${interface}" in
		...
		lo)
			...
			# CAKE
			#tc qdisc replace dev "${interface}" root cake rtt 20ms flows besteffort ack-filter no-split-gso memlimit 32MB
			# SFQ
			#tc qdisc replace dev ${1} root sfq ecn
			...
			ip a add 127.1.0.1/8 label "${interface}":dnscrypt-1 dev "${interface}"
			ip a add 127.1.0.2/8 label "${interface}":dnscrypt-2 dev "${interface}"
			ip a add 127.1.0.3/8 label "${interface}":dnscrypt-3 dev "${interface}"
			ip a add 127.1.0.4/8 label "${interface}":dnscrypt-4 dev "${interface}"
			ip a add 127.1.0.5/8 label "${interface}":dnscrypt-5 dev "${interface}"
			ip a add 127.1.0.6/8 label "${interface}":dnscrypt-6 dev "${interface}"
		;;
		# WiFi
		wl*)
			# it's recommended to "set quantum to the size of 1 or 2 mtu" to eliminate possible interference with RT tasks
			ethtool -K "${interface}" tx-nocache-copy on gro on
			# FQ_CODEL
			#tc qdisc replace dev "${interface}" root fq_codel limit 16384 flows 5120 target 20ms interval 125ms quantum 2327 ecn
			# CAKE
			tc qdisc replace dev "${interface}" root cake rtt 125ms flows diffserv4
		;;
		# Ethernet
		e*)
			...
			ethtool -K "${interface}" tx-nocache-copy off rx on tx on sg on ufo on gro on gso off tso on lro on rxvlan on txvlan on ntuple on rxhash on
			...
			# "rx-frames 0 tx-frames 0 rx-usecs 125 tx-usecs 250" seems like decent compromise
			ethtool -C "${interface}" rx-frames 4096 tx-frames 1024 rx-usecs 1000 tx-usecs 250 adaptive-rx on adaptive-tx on
			#ethtool -C "${interface}" rx-frames 0 tx-frames 0
			# FQ_CODEL
			#tc qdisc replace dev "${interface}" root fq_codel limit 16384 flows 32768 target 15ms interval 100ms quantum 3028 ecn
			# CAKE
			tc qdisc replace dev "${interface}" root cake ethernet ether-vlan rtt 100ms flows diffserv4 split-gso
		;;
		# else
		*)
			# FQ_CODEL
